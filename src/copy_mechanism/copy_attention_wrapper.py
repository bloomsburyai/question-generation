...
class AttentionWrapperState(
    collections.namedtuple("AttentionWrapperState",
                           ("cell_state", "attention", "time", "alignments",
                            "alignment_history", "copy_alignments"))):
...
class CopyAttentionWrapper(rnn_cell_impl.RNNCell):
  ...
  def state_size(self):
    ...
    return AttentionWrapperState(
        ...
        copy_alignments=self.copy_mechanism.alignments_size)  # sometimes a TensorArray
    ...
    with ops.name_scope(type(self).__name__ + "ZeroState", values=[batch_size]):
      ...
      return AttentionWrapperState(
          ...
          alignment_history=self._item_or_tuple(
              ...
              for _ in self._attention_mechanisms),
          copy_alignments=self.copy_mechanism.initial_alignments(batch_size, dtype))
  ...
  def call(self, inputs, state):
    ...
    copy_alignments = self.copy_mechanism(
        cell_output, previous_alignments=state.copy_alignments)
    ...
    # alignments: [batch_size, seq_length] attention weights
    # attention:  [batch_size, embedding_dim] context vectors

    #attention = tf.Print(attention, [
    #    tf.shape(cell_output),
    #    tf.shape(attention),
    #    tf.shape(alignments)
    ...
    memory = self._attention_mechanisms[0]._values

    # todo: reconstruct word ids from memory and weight by alignments
    # ideally we would get access to the word ids of the input instead of their embeddings in "memory"
